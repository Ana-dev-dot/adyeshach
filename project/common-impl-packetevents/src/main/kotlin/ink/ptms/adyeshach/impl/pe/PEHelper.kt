...
class PEHelper : MinecraftHelper {
    ...
    companion object {
        fun safeItem(item: org.bukkit.inventory.ItemStack?): com.github.retrooper.packetevents.protocol.item.ItemStack {
            if (item == null || item.type.isAir || item.type.name.contains("AIR")) {
            ...
            return try {
                ...
                try {
                    val peType = com.github.retrooper.packetevents.protocol.item.type.ItemTypes.getByName(item.type.name.lowercase())
                        ?: com.github.retrooper.packetevents.protocol.item.type.ItemTypes.STONE
                    com.github.retrooper.packetevents.protocol.item.ItemStack.builder().type(peType).amount(item.amount).build()
                } catch (e2: Throwable) {
